...
\end{lemma}
	...
	\frac{\partial}{\partial R}(L^R(R)/L(R))=\frac{\mu  \left(\mu  (m+1) \log \left(\frac{\mu +\frac{2 (m+2) R^2}{m+1}}{\mu +\frac{2 m R^2}{m+1}}\right)+4 R^2\right)}{4 \pi  R^2 \left(\mu +\frac{m (m+2) R^2}{(m+1)^2}\right)^{3/2}}
	\end{equation}
	...
	\end{equation}
	 ...
	 This is a decreasing function of $ m $, as is verified by considering the derivative\begin{equation}
	 ...
	 \end{equation}
	 Knowing that $ L^R(k)/L(k)\rvert_{R\geq(1+m)\sqrt{\mu}}\geq h(m,\mu) $ and that $ h(m,\mu) $ is non-increasing, we need only consider the limit $ L^R(R)/L(R)\big\rvert_{R\geq(1+m)\sqrt{\mu}}\geq\lim\limits_{m\to\infty}\left(h(m,\mu)\right)=\frac{1}{2} $.
	\end{proof}
...
\end{proof}
\section{Monotonicity of $ L^R(k)/L(k)$ for $ k\leq R $ }
