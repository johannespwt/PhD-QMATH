...
%		
	...
	We will now generalize the results from the previous subsection. The problem is analogous to the problem in the $ 2+1 $ case, except for the substitution $ k\to\abs{\sum_{i=1}^{N-1}k_i} $ and $ \mu\to\mu- $.
	
	Use that $ \xi^R\in H^1 $ so we have better large $ q $ behavior than Moser and Seiringer, and remember that Moser and Seiringer use symmetry of $ \xi $.
