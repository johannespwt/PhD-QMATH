...
%	\end{lemma}
	...
	\begin{proof}
		...
		\abs{\abs{\phi(x)}-\abs{\phi(x')}}^2\leq\abs{\phi-\phi(x')}^2\leq R\left(\int_{[0,R]}\abs{\partial \phi}^2\right),
		\end{equation}
		...
		\end{equation}
	\end{proof}
	Choosing $ R\geq 2\abs{a} $ we clearly see that $ \braket{\psi|\psi}\geq 1- \text{const. }R^2 E $.\\
	
%	Now we may optimize the right-hand side of \eqref{Eqmaxbound} in $ g $. Doing this, we find $ g=\frac29 \frac{1}{R} $, thus we choose $ g=\min(\frac{1}{2}\int v, \frac29 \frac{1}{R}) $ and we have $ \min(\frac{2}{R-a},\frac{2}{9R})\leq g\leq \frac{2}{9}\frac{1}{R} $ ($ g=\frac{2}{9R} $ if $ R\gg \abs{a} $) with \begin{equation}\label{Eqmaxbound2}
%	\int_{[x_j-R,x_j+R]}\abs{\partial_i\psi}^2+\frac12v_{ij}\abs{\psi}^2 \diff x_i\geq \underbrace{\frac{2}{27}\frac{1}{R}}_{:=\tilde{g}}\max_{\abs{x_j-x_i}<R}\abs{\psi}^2
%	\end{equation}
%	Now let $ \Psi $ be the groundstate of $ \mathcal{E} $, with energy $ E $. Defining $ \psi\in L^2([0,\ell-(n-1)R]^n) $ by $$ \psi(x_1,...,x_n)=\Psi(x_1,R+x_2,...,(n-1)R+x_n), $$
%	for $ x_1\leq x_2\leq....\leq x_n $. We seek to estimate the norm of $ \psi $ knowing that $ \norm{\Psi}=1 $. It is clear that \begin{equation}
%	\begin{aligned}
%	\braket{\psi|\psi}&=\int \abs{\Psi}^2 \prod_{i< j}\theta(\abs{x_i-x_j}-R)\\&\geq 1-\int \abs{\Psi}^2\left( \sum_{i<j}(1-\theta(\abs{x_i-x_j}-R))\right)\\
%	&=1-\int \abs{\Psi}^2\left( \sum_{i<j}\theta(R-\abs{x_i-x_j})\right)
%	\end{aligned}
%	\end{equation}
%	where $ \theta $ is the Heaviside step-function.
%	Clearly we have \begin{equation}
%	\begin{aligned}
%	\int \abs{\Psi}^2\sum_{i<j}\theta(R-\abs{x_i-x_j})&\leq 2R\int  \sum_{i< j}\max_{\abs{x_i-x_j}<R}\abs{\Psi}^2\diff\bar{x}^i\\
%	&\leq\frac{R}{\tilde{g}} \int\sum_{i\neq j}\int_{[x_j-R,x_j+R]} \abs{\partial_i\psi}^2+\frac12v_{ij}\abs{\psi}^2 \diff x_i\diff\bar{x}^i\\
%	&\leq \frac{R}{\tilde{g}} E\leq \text{const. }n^2R^2\rho^2,
%	\end{aligned}
%	\end{equation}
%	where the second inequality follows from \eqref{Eqmaxbound2} the last inequality follows by the upper bound from the previus section. Hence we have $ \braket{\psi|\psi}\geq 1-\text{const. }n^2(\rho R)^2 $.\\
%	
...
%	
%	\begin{lemma}\label{LemmaPotentialSplit}
%		Let $ n\gg 1 $, then for any positive potential, $ v\geq0 $, there exist a decomposition $ v=v_1+v_2 $  with $ v_1,v_2\geq0 $ such that $ v_1 $ has scattering length $a_1= a+(R-a)\mathcal{O}(1/n) $ and $ \int v_2=\frac{C}{n(R-a)} $
%	\end{lemma}
%	\begin{proof}
%		Let $ v_2\geq 0 $ be such that $ \int v_2=\frac{1}{n(R-a)} $ and $ v_1:=v-v_2\geq 0 $. Then for $ \phi $ equal to the minimizer of $ \int_{[-R,R]}\abs{\partial\phi}^2+\frac12v_1\phi $ with $ \phi(R)=1 $ we have $ \frac{4}{R-a}\geq\int\abs{\partial\phi}^2+\frac{1}{2}v_1\phi\geq\int\abs{\partial\phi}^2+\frac{1}{2}v\phi-\frac12\int v_2=\frac{4}{R-a}-\frac{1}{2}\frac{C}{n(R-a)}= \frac{4}{R-a}(1-\frac{C}{8n})\geq\frac{4}{R-a+(R-a)\mathcal{O}(1/n)}  $.
%	\end{proof}
%	By this lemma, we may consider the potential as being a sum of two terms, one with almost unchanged scattering lengt, and one with $ \int v_2=\frac{C}{n(R-a)} $.\\
	...
	\begin{theorem}
		...
		E\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+C_\epsilon(\rho R)^{4/3-\epsilon}\right)
		\end{equation}
	...
	\begin{proof}
		...
		&\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }\left(\frac{1}{n^{2/3}}+n^k(\rho R)^{2k}+...+n^{1/3}(\rho R)^{2}\right)\right)
		\end{aligned}
		\end{equation}
		Choosing $ n=(\rho R)^{-6k/(3k+2)} $ we have \begin{equation}
		E\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }(\rho R)^{12k/(9k+6)}\right).
		\end{equation}
		Choosing $ k $ large enough so that $ 12k/(9k+6)\geq 4/3-\epsilon $, the desired result follows. Notice that the constant will be dependent on the choice of $ k $ and thus $ \epsilon $.
	\end{proof}
	\subsection{Lieb-Liniger model lower bound}
	In the proof above, we use that the Lieb Liniger energy is lower bounded by $ E_{LL}^{\rho,\ell,c}\geq \frac{\pi^2}{3}\rho^2(1+4\rho/c-\text{const. }N^{-2/3}) $. This we will show in this subsection we show this lower bound.\\
	We use that the energy in the Lieb Liniger model, in the thermodynamic limit, is determined by the sytem of equation ((3.3) and (3.18)--(3.20) in \cite{PhysRev.130.1605})
	\begin{align}
	...
	\end{align}
	Neglecting $ (x-y)^2 $ in the denominator of \eqref{Eq3}, we see that $ g\leq \frac{1}{2\pi}+2\frac{1}{\lambda}\int_{-1}^{1}g(x)\diff x $. On the other hand $ \eqref{Eq4} $ shows that $ e(\gamma)=\frac{\int_{-1}^{1}g(x)x^2\diff x}{\left(\int_{-1}^{1}g(x)\diff x\right)^3} $. Hence we denote $ \int_{-1}^{1}g(x)\diff x=M $, and notice that we have $ g\leq \frac{1}{2\pi}\left(1+\frac{2M}{\lambda}\right) $. It is now easily verified that, $ \int_{-1}^{1}g(x)x^2\diff x $ with $ M $ and $ g\leq \frac{1}{2\pi}\left(1+\frac{2M}{\lambda}\right)=\frac{1}{2\pi}\left(1+2\gamma^{-1}\right) $ is mininmized by $ g=K\chi_{[-M/(2K),M/(2K)]} $, with $ K=\frac{1}{2\pi}\left(1+\frac{2}{\gamma}\right) $. This gives us $ \int_{-1}^{1}g(x)x^2\diff x=\frac{M^3}{3K^2}$ so that we have $ e(\gamma)\geq \frac{1}{3K^2}=\frac{\pi^2}{3}\left(\frac{\gamma}{\gamma+2}\right)^2\geq \frac{\pi^2}{3}(1-\frac{4}{\gamma})$ for $ \gamma>0 $.\\
	Now to pass to a finite box, we will use the result by Robinson, \cite{robinson2014thermodynamic} that for any $ b>0 $ \begin{equation}\label{EqRobinsonBound}
	E^N(N,L)\geq E^D(N,(L+b))-\text{const. }\frac{N}{b^2}.
	\end{equation}
	\begin{lemma}
		For the Lieb-Liniger model we have \begin{equation}
		\begin{aligned}
		e^D_{LL}(2^mN,2^mL)&\downarrow e_{LL}(\rho),\quad \text{  }m\to\infty,\\
		e^N_{LL}(2^mN,2^mL)&\uparrow e_{LL}(\rho),\quad \text{  }m\to\infty
		\end{aligned}
		\end{equation}
	\end{lemma}
	where superspcript $ D$/$N $ refers to Dirichlet/Neumann boundary conditions and where $ e_{LL}(\rho) $ denotes the thermodynamic limit in the periodic boundary condition case.
	\begin{proof}
		Notice that since the interaction in the Lieb-Liniger model has no range, we can for the Dirichlet energy, $ E_{LL}^D(2^{m+1}N,2^{m+1}L) $ use the trial state consistion of the product of ground states for the $(2^{m}N,2^{m}L)$ systems. Then we obviously get $ E_{LL}^D(2^{m+1}N,2^{m+1}L)\leq E_{LL}^D(2^{m}N,2^{m}L)+E_{LL}^D(2^{m}N,2^{m}L) $ such that the energy per particle satisfies \begin{equation}
		e_{LL}^D(2^{m+1}N,2^{m+1}L)=\frac{E_{LL}^D(2^{m+1}N,2^{m+1}L)}{2^{m+1}}\leq \frac{1}{2}\left(\frac{2E^{D}_{LL}(2^mN,2^mL)}{2^m}\right)=e_{LL}^D(2^{m}N,2^{m}L).
		\end{equation}
		On the other hand, for the Neumann energy, $ E^n_{LL}(2^{m+1}N,2^{m+1}L) $, we can localize in two smaller boxes of length $ 2^m L $. Throwing away interactions between the different boxes the energy functional completely localizes in the boxes. Thus we get \begin{equation}
		E^n_{LL}(2^{m+1}N,2^{m+1}L)\geq \inf_{\{M_i\}} \sum_{i=1}^{2}E^n_{LL}(M_i,2^mL)=2E^n_{LL}(2^{m}N,2^{m}L),
		\end{equation}
		where the infimum is running over $ \{M_1,M_2\} $ with $ M_1+M_2=2^{m+1}N $. Hence we have established monotonicity. That they converge to the thermodynamic limit (as defined for now by the periodic boundary condition limit), follows from the fact that the $ D $/$ N $ limits are bounded below/above by the limit from periodic boundary condtions, which is the case analyzed in \cite{PhysRev.130.1605} but at the same time the Robinson bound \eqref{EqRobinsonBound} ensures that $ e_{LL}^N(\rho) $ is bounded below $ e_{LL}\left(\rho\left(1+b/L\right)^{-1}\right)-\frac{1}{b^2} $ and conversely that $ e_{LL}^D(\rho) $ is bounded above by $ e_{LL}\left(\rho\left(1-b/L\right)^{-1}\right)$ for any $ L $ and any $ b\ll L $. Using the known result, $ e_{LL}(\rho)= $
	\end{proof}
	\begin{proof}[Alternative proof]
		Define $ f(\mu,L)=\inf_\rho e(\rho,L)-\rho\mu $, where $ e=E/L $. Then we have by \eqref{EqRobinsonBound} \begin{equation}\label{EqRobinsonBound2}
		f_{LL}^N(\mu,L)\geq f_{LL}^D(\mu+\text{const. }1/b^2,L+b)(1+b/L).
		\end{equation}
		Now clearly $ f_{LL}^D(\mu,2^m L) $ is decreasing in $ m $, and $ f_{LL}^N(\mu,2^m,L) $ is increasing in $ m $. Furthermore, since $ f_{LL}^D\geq f_{LL}\geq f_{LL}^N $ we have that $ \lim\limits_{m\to\infty}f_{LL}^D(\mu,2^m L)\geq f_{LL}(\mu)\geq  \lim\limits_{m\to\infty}f_{LL}^N(\mu,2^m L)   $. By \eqref{EqRobinsonBound2} we have $ f_{LL}^N(\mu)\geq f_{LL}^D(\mu+\text{const. }1/b^2) $ for any $ b $. Hence by continuity of $ f_{LL}^D $ we have $ f_{LL}^D(\mu)=f_{LL}^N(\mu)=f_{LL}(\mu) $. Thus by \eqref{EqRobinsonBound2} we have \begin{equation}
		\begin{aligned}
				e^D_{LL}(\rho,L)\geq e_{LL}(\rho)\geq e_{LL}^N(\rho,L)\geq e^D_{LL}\left((\rho(1+b/L)^{-1},L+b\right)(1+b/L)-\rho/b^2\\
				\geq e_{LL}\left((\rho(1+b/L)^{-1}\right)(1+b/L)-\rho/b^2 
		\end{aligned}
		\end{equation}
		Using the known bound $ e_{LL}(\rho)\geq \frac{\pi^2}{3}\rho^3\left(\frac{\gamma}{\gamma+2}\right)^2 $
		we find \begin{equation}
		e^N_{LL}(\rho,L)\geq \frac{\pi^2}{3}\rho^3\left(1-\text{const. }(b/L+1/(\rho b)^2)\right)\left(\frac{\gamma}{\gamma+2}\right)^2
		\end{equation}
	\end{proof}
	Using that the Dirichlet energy per particle, $ e^D_{LL}(2^mN,2^mL) $ is decreasing towards the thermodynamic limit $ e_{LL}(\rho) $ as $ m\to \infty $, and the Neumann energy per particle, $ e_{LL}^N(2^mN,2^mL) $ is increasing towards the thermodynamic limit $ e_{LL}^n(\rho) $ as $ m\to\infty $, we see that \begin{equation}
	E^N_{LL}(N,L)\geq E_{LL}\left(\rho\left(1+\frac{b}{L}\right)^{-1}\right)-\text{const. }\frac{N}{b^2}.
	\end{equation}
	For the Lieb-Liniger model we have $ E_{LL}(\rho)\geq\frac{\pi^2}{3}\rho^2\left(\frac{\gamma}{\gamma+2}\right)^2 $, and hence for $ b\ll L $ \begin{equation}
	E_{LL}^N(N,L)\geq N\frac{\pi^2}{3}\rho^2(1-\text{const. }b/L)-\text{const. }\frac{N}{b^2}.
	\end{equation}
	Optimizing in $ b $, we find that \begin{equation}
	E_{LL}^N(N,L)\geq N\frac{\pi^2}{3}\rho^2\left(1+4\rho/c+\text{const. }N^{-2/3}\right)
	\end{equation}
