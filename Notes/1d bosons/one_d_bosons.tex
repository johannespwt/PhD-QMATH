...
%		\end{equation}
%	\end{lemma}
	\begin{lemma}[Sobolev's inequality]
	...
	\end{proof}
		...
		\begin{equation}
		\begin{aligned}
		\braket{\psi|\psi}=1-\int_{B}\abs{\Psi}^2=\sum_{i<j}\int_{B_{ij}}\abs{\Psi}^2
		\end{aligned}
		\end{equation}
		...
		\braket{\psi |\psi }\geq 1- \sum_{i<j}\int R \max_{\mathfrak{r}_i(x)=\abs{x_i-x_j}<R}\abs{\Psi}^2 \diff \bar{x}^i\geq 1-R^2\sum_{i<j}\int_{B_{ij}}\abs{\partial\Psi}^2\geq 1- R^2 E\geq 1-\text{const. }n(\rho R)^2.
		\end{equation}
	...
	\begin{proof}
		...
		\max_{x\in[0,R]}\abs{\phi-\phi(x')}^2\leq R\left(\int_{[0,R]}\abs{\partial \phi}^2\right),
		\end{equation}
		for $ x'\in[0,R] $. It follows that \begin{equation}
		\abs{\phi(x')}^2\geq \max_{x\in[0,R]}\abs{\phi}^2-R\int_{[0,R]}\abs{\partial \phi}^2
		\end{equation}
		Viewing $ \Psi $ as a function of $ x_i $ we have \begin{equation}
		\min_{\mathfrak{r}_i(x)=\abs{x_i-x_j}<R}\abs{\Psi}^2\geq \max_{\mathfrak{r}_i(x)=\abs{x_i-x_j}<R}\abs{\Psi}^2-2R\left(\int_{{\mathfrak{r}_i(x)=\abs{x_i-x_j}<R}}\abs{\partial_i \Psi}^2\right).
		\end{equation}
		Hence we find \begin{equation}
		\begin{aligned}
		&\sum_{i<j}\int v_{ij} \abs{\Psi}^2\geq \sum_{i<j} \int_{B_{ij}} v_{ij} \abs{\Psi}^2 \\&\geq \left(\int v\right)\sum_{i< j}\int\left(\max_{B_{ij}}\abs{\Psi}^2-2R\left(\int_{B'_{ij}}\abs{\partial_i\Psi}^2\diff x_i\right)\right)\diff \bar{x}^i\\
		&\geq \frac{4}{R-a}\sum_{i< j}\left(\frac{1}{2R}\int_{B_{ij}}\abs{\Psi}^2-2R\int_{B_{ij}}\abs{\partial_i\Psi}^2\right)\diff \bar{x}^i
		\end{aligned}
		\end{equation}
		where $ B_{ij}=\{x\in \R^n \vert \mathfrak{r}_i(x)=\abs{x_i-x_j}<R \} $ and $ B'_{ij}=\{x_i\in \R \vert \mathfrak{r}_i(x)=\abs{x_i-x_j}<R \} $. Now noticing that $ \sum_{i< j}\int_{B_{ij}}\abs{\Psi}^2=1-\braket{\psi|\psi} $ we see that
		\begin{equation}
		1-\braket{\psi|\psi}\leq \text{const. } \left(R^2\sum_{i<j}\int_{B_{ij}}\abs{\partial_i \Psi}^2+R(R-a)\int\sum_{i<j} v_{ij} \abs{\Psi}^2\right)
		\end{equation}
	\end{proof}
	Choosing $ R\gg a $ we clearly see that $ \braket{\psi|\psi}\geq 1- \text{const. }R^2 E $.\\
...
%	
	...
	This lemma will essentially allows us to replace the potential by a shell potential of range $ R $ and strength $ \frac{2}{R-a} $, where $  $
	 This bring us to the result 
%	 	\begin{theorem}[Fermi lower bound]
%	 		Let $ n\rho R\ll 1 $, then
%	 		\begin{equation}
...
%	 		E=\mathcal{E}(\Psi)\geq \int\sum_{i}\abs{\partial_i\Psi}^2\chi_{\mathfrak{r}_i(x)>R}+\frac{1}{2}\sum_{i}\frac{2}{R-a}(\delta(\mathfrak{r}_i(x)+R)+\delta(\mathfrak{r}_i(x)-R))\abs{\Psi}^2,
%	 		\end{equation}
%	 		where $ \mathfrak{r}_i(x) $ denotes the distance from $ x_i $ to its nearest neighbor. Defining $ \varphi\in L^2([0,\ell-(n-1)R]^n) $ by $$ \psi(x_1,...,x_n)=\Psi(x_1,R+x_2,...,(n-1)R+x_n), $$
...
%	 		\begin{aligned}
%	 		E&\geq n\frac{\pi^2}{3}\rho^2\left(1-2\rho(R-a)+2\rho R+\text{const. }\frac{1}{n}\right)(1-\text{const. }n(\rho R)^2)\\&=n\frac{\pi^2}{3}\rho^2\left(1-2\rho(R-a)+2\rho R-\text{const. }\left(n(\rho R)^2+\frac{1}{n}\right)\right)\\
%	 		&\geq n\frac{\pi^2}{3}\rho^2\left(1-\textnormal{const. }\left(\rho a +n(\rho R)^2+\frac{1}{n}\right)\right).
%	 		\end{aligned}
%	 		\end{equation}
%	 		
%	 		Choosing $ n=(\rho R)^{-1} $ we see that \begin{equation}
%	 		E\geq n\frac{\pi^2}{3}\rho^2(1-\text{const. }(\rho R))
%	 		\end{equation}
%	 	\end{proof}
	 
	 \begin{theorem}[General lower bound]
		Let $ n(\rho R)^2\ll 1 $. Then\begin{equation}
		E\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a +\textnormal{const. }\left(n(\rho R)^2+\frac{1}{n}\right)\right)
		\end{equation}
	\end{theorem}
	\begin{proof}
		We consider $ E=\mathcal{E}(\Psi)=\int\sum_{i}\abs{\partial_i\Psi}^2+\frac{1}{2}\sum_{i\neq j}v_{ij}\abs{\Psi}^2 $. By Dyson's lemma, we may estimate\begin{equation}
		E=\mathcal{E}(\Psi)\geq \int\sum_{i}\abs{\partial_i\Psi}^2\chi_{\mathfrak{r}_i(x)>R}+\frac{1}{2}\sum_{i}\frac{2}{R-a}(\delta(\mathfrak{r}_i(x)+R)+\delta(\mathfrak{r}_i(x)-R))\abs{\Psi}^2,
		\end{equation}
		where $ \mathfrak{r}_i(x) $ denotes the distance from $ x_i $ to its nearest neighbor. Defining $ \psi\in L^2([0,\ell-(n-1)R]^n) $ by $$ \psi(x_1,...,x_n)=\Psi(x_1,R+x_2,...,(n-1)R+x_n), $$
		for $ x_1\leq x_2\leq....\leq x_n $, we see that \begin{equation}
		E=\mathcal{E}_{LL}^{\tilde{\rho},\tilde{\ell},c}(\psi)\geq E_{LL}^{\tilde{\rho},\tilde{\ell},c}\braket{\psi|\psi},
		\end{equation}
		where $ \mathcal{E}_{LL}^{\tilde{\rho},\tilde{\ell},c} $ is the Lieb-Liniger functional in volumen $ [0,\tilde{\ell}]^n $ with density $ \tilde{\rho} $ and coupling $ c $, and we have $ \tilde{\ell}=\ell-(n-1)R $, $ \tilde{\rho}=\frac{n}{\tilde{\ell}}\approx\rho (1+\rho R) $ and $ c=\frac{2}{R-a} $. Given that $ \tilde{\rho}/c\ll 1 $ we have for finite $ n $ \begin{equation}
		E_{LL}^{\tilde{\rho},\tilde{\ell},c}=n\frac{\pi^2}{3}\tilde{\rho}^2\left(1-4\tilde{\rho}/c+\text{const. }\frac{1}{n}\right)\approx n\frac{\pi^2}{3}\rho^2\left(1-2\rho(R-a)+2\rho R+\text{const. }\frac{1}{n}\right)
		\end{equation}
		Using the result above, lemma \ref{LemmaNormLoss} , on $ \braket{\psi|\psi} $, we find \begin{equation}
		\begin{aligned}
		E&\geq n\frac{\pi^2}{3}\rho^2\left(1-2\rho(R-a)+2\rho R+\text{const. }\frac{1}{n}\right)(1-\text{const. }n(\rho R)^2)\\&=n\frac{\pi^2}{3}\rho^2\left(1-2\rho(R-a)+2\rho R-\text{const. }\left(n(\rho R)^2+\frac{1}{n}\right)\right)\\
		&\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a-\textnormal{const. }\left( +n(\rho R)^2+\frac{1}{n}\right)\right).
		\end{aligned}
		\end{equation}
		...
		\end{equation}
	\end{proof}
	By a localization argument, this bound holds for any system with $ N^{-1}\ll \rho R\ll N $ particles.
...
%	By this lemma, we may consider the potential as being a sum of two terms, one with almost unchanged scattering lengt, and one with $ \int v_2=\frac{C}{n(R-a)} $.\\
	...
	&\geq E_{LL}^{\tilde{\rho},\tilde{\ell},\tilde{c}=\frac{2\epsilon}{R-a}}\braket{\psi|\psi}+ (1-\epsilon)(1-\braket{\psi|\psi})
	\end{aligned}
	\end{equation}
	From this and the known upper bound, it follows that 
	\begin{equation}\label{EqMassBound}
	\begin{aligned}
	 1-\braket{\psi|\psi} \leq \text{const. }\frac{1}{(1-\epsilon)}\frac{1}{\epsilon}n\rho^2 R((\rho R)+\rho(R-a))=\text{const. }n\rho^2R^2(\rho R+\rho(R-a))
	\end{aligned}
	\end{equation}
	where $ B=\{x\in\R^n\ \vert\ \min_{(i,j)}\abs{x_i-x_j}<R \} $.\\
	\begin{theorem}
		We have \begin{equation}
		E\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\textnormal{const. }(\rho R)^{3/2}\right)
		\end{equation}
	\end{theorem}
	\begin{proof}
		...
		n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }\frac{1}{n}\right)\left(1-\text{const. }n(\rho R)^{3}\right)\\
		&\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }\frac{1}{n}+n(\rho R)^{3}\right)
		\end{aligned}
		\end{equation}
		Choosing $ n=(\rho R)^{-3/2} $ we have \begin{equation}
		E\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }(\rho R)^{3/2}\right)
		\end{equation}
	\end{proof}
	By a localization argument, this proves that $ E\geq N\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\mathcal{O}\left((\rho R)^{3/2}\right)\right) $ for any $ N $ with $ N\gg (\rho R)^{-3/2}\gg N^{-1} $.
	\end{document}