...
\begin{document}
	...
	\int_{[x_j-R,x_j+R]} \frac{1}{n-1}\abs{\partial_i\psi}^2+\frac12v_{ij}\abs{\psi}^2 \diff x_i\geq \underbrace{\frac{1}{9}\left(1-\frac{\sqrt{2}}{3}\right)\frac{1}{(n-1)R}}_{:=\tilde{g}}\max_{\abs{x_j-x_i}<R}\abs{\psi}^2
	\end{equation}
	...
	&\leq\frac{R}{\tilde{g}} \int\sum_{i\neq j}\int_{[x_j-R,x_j+R]} \frac{1}{n-1}\abs{\partial_i\psi}^2+\frac12v_{ij}\abs{\psi}^2 \diff x_i\diff\bar{x}^i\\
	...
	\begin{proof}
		...
		Choosing $ n=(\rho R)^{-2/3} $ we see that \begin{equation}
		E\geq n\frac{\pi^2}{3}\rho^2(1-\text{const. }(\rho R)^{2/3})
		\end{equation}
	\end{proof}
...
%	By this lemma, we may consider the potential as being a sum of two terms, one with almost unchanged scattering lengt, and one with $ \int v_2=\frac{C}{n(R-a)} $.\\
	...
	By splitting $ \int_{[-R,R]}\abs{\partial\phi}^2+\frac12 v\abs{\phi}^2 $, and use Dyson's lemma on one term and Lieb, Seiringer, Yngvason on the other, we get\begin{equation}
	\begin{aligned}
	&\int \sum_{i}\abs{\partial_i\Psi}^2+\sum_{i\neq j} \frac{1}{2}v_{ij}\abs{\Psi}^2\geq\\ &\int\sum_{i}\abs{\partial_i\Psi}^2\chi_{\mathfrak{r}_i(x)>R}+\epsilon\sum_{i}\frac{2}{R-a}(\delta(\mathfrak{r}_i(x)+R)+\delta(\mathfrak{r}_i(x)-R))\abs{\Psi}^2+\sum_{i\neq j} (1-\epsilon)\tilde{g}\max_{\abs{x_j-x_i}<R}\abs{\Psi}^2\\
	&\geq E_{LL}^{\tilde{\rho},\tilde{\ell},\tilde{c}=\frac{2\epsilon}{R-a}}\braket{\psi|\psi}+\sum_{i\neq j} (1-\epsilon)\tilde{g}\max_{\abs{x_j-x_i}<R}\abs{\Psi}^2
	\end{aligned}
	\end{equation}
	From this and the known upper bound, it follows that 
	\begin{equation}\label{EqMassBound}
	\begin{aligned}
	 \int_{B}\abs{\Psi}^2 \leq R\int\sum_{i\neq j} \max_{\abs{x_j-x_i}<R}\abs{\Psi}^2\diff \bar{x}^i&\leq\text{const. }\frac{1}{\tilde{g}(1-\epsilon)}\frac{1}{\epsilon}\rho^2 R((\rho R)^{2/3}+\rho(R-a))\\&=\text{const. }n\rho^2R^2((\rho R)^{2/3}+\rho(R-a))
	\end{aligned}
	\end{equation}
	where $ B=\{x\in\R^n\ \vert\ \min_{(i,j)}\abs{x_i-x_j}<R \} $.\\
	\begin{theorem}
		We have \begin{equation}
		E\geq n\frac{\pi^2}{3}\rho^2(1+2\rho a+\textnormal{const. }(\rho R)^x)
		\end{equation}
	\end{theorem}
	\begin{proof}
		...
		\begin{aligned}
		E&\geq \mathcal{E}_{LL}^{\tilde{\rho},\tilde{\ell},c}(\psi)\geq E_{LL}^{\tilde{\rho},\tilde{\ell},c}\braket{\psi|\psi}=E_{LL}^{\tilde{\rho},\tilde{\ell},c}\left(1-\int_B\abs{\Psi}^2\right)\\&\geq
		n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }\frac{1}{n}\right)\left(1-\text{const. }n(\rho R)^{8/3}\right)\\
		&\geq n\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\text{const. }\frac{1}{n}+n(\rho R)^{8/3}\right)
		\end{aligned}
		\end{equation}
	\end{proof}
	\end{document}