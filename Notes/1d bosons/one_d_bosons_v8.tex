...
%			from which \eqref{EqLocalizationF} follows. 
		...
		\end{proof}
		\section{Ground state energy for the Bose and Fermi gasses}
		From Theorems \ref{TheoremUpperBound} and \ref{TheoremLowerBound} the following result follows straightforwardly \begin{corollary}
			...
			E(N,L)=N\frac{\pi^2}{3}\rho^2\left(1+2\rho a+\mathcal{O}\left(\rho R\right)^{6/5}\right),
			\end{equation}
		...
		\section{A remark on anyons}
			...
			\end{cases} $ and $ \vert^{ij}_{\pm,0} $ means the function evaluated at $ x_i=x_{j \pm,0} $. Then the following proposition holds \begin{proposition}\label{PropositionAnyonQuadraticForm}
				...
				\mathcal{E}_\kappa(\varphi)=\sum_{i=1}^{N}\int_{{\R^N\setminus\bigcup_{i<j}\{x_i=x_j\}}} \abs{\partial_{x_i}\varphi(x)}^2+\frac{2c}{\cos(\kappa/2)}\sum_{i<j} \delta(x_i-x_j)\abs{\varphi(x)}^2\diff^{N}x
				\end{equation}
			\end{proposition}
			\begin{proof}
			...
			\end{proof}
			In this setting, the results for bosons and fermions follows as special cases.
		 \begin{corollary}
		 	...
		 	\end{equation}
		 	where $ a_{\pi}=a_o $ is the odd wave scattering length.
		 \end{corollary}
		 \begin{proof}
		 	This follows straigthforwardly by noticing that the phase-factor and the boundary condition imposed at the planes of intersection, exactly ensures that $ \dom{\overline{\mathcal{E}_\pi}}=\dom{\mathcal{E}_F} $.
		 \end{proof}
