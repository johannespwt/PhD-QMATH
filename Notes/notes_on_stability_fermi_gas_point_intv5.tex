...
\begin{lemma}\label{lemmaGxi} Let $ N\geq2 $ and $ \xi\in L^2(\R^{3(N-1)}) $, then we have the following bound
	\begin{equation}
	\norm{\frac{1}{n}\int_{B_n(0)}\diff k_N\mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)}_{L^2(\R^{3(N-1)})}\leq C\norm{\xi}_{L^2(\R^{3(N-1)})},
	\end{equation}
	where $ C>0 $ depends only on $ N $, $ m $, and $ \mu $, for all $ 1\leq i\leq N $.
\end{lemma}
\textbf{Remark}: Notice that the proof below appears to be generalizable to the case $ \xi\in H^{1/2}(\R^{3(N-1)}) $ in which case we have 
\begin{equation}
\norm{\frac{1}{n}\int_{B_n(0)}\diff k_N\mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)}_{H^{1/2}(\R^{3(N-1)})}\leq C\norm{\xi}_{H^{1/2}(\R^{3(N-1)})},
\end{equation}
for all $ 1\leq i\leq N $. However, the generalization have not been done in detail yet.
\begin{proof}
