...
\mathscr{D}(F_\alpha^R)=\left\{u\in H_{\text{as}}^1(\R^{3N})\ \Big\vert\ \int_{B_R(0)}\diff k_N \hat{u}(k)\in H_{\text{as}}^{1/2}\left(\R^{3(N-1)}\right)\right\}.
\end{equation}
...
\end{aligned}
\end{equation}
where we have defined $ \widehat{\rho G}=\sum_{n=1}^{N}(-1)^{i-1}\hat{G}(k)\xi(\bar{k}^i) $, and \begin{equation}
\begin{aligned}
