...
\end{equation}
Since we know that $ \abs{\xi^n-\xi}\rightharpoonup0 $ in $ H^{1/2}(\R^{3(N-1)}) $ (Similar argument to the one above for $ \xi_n\rightharpoonup\xi $, just observe that $ \abs{\xi^n-\xi} $ $ H^{1/2}(\R^{3(N-1)}) $ bounded and converge to zero in $ L^{2}(\R^{3(N-1)}) $ is ). Thus we know\begin{equation}
\begin{aligned}
