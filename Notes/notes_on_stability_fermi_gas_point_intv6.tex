...
\end{proof}
A direct consequence is the following lemma \begin{lemma}\label{lemmawH1-2}
...
\begin{proof}
	...
	\leq\frac{1}{n}\int_{\abs{\bar{k}^N}<n}\diff\bar{k}^N\sqrt{\abs{\bar{k}^N}^2+1}\abs{g(\bar{k}^N)}^2+\frac{1}{n}\int_{\abs{\bar{k}^N}\geq n}\diff \bar{k}^N \sqrt{\abs{\bar{k}^N}^2+1}\frac{n^2}{(\abs{\bar{k}^N}^2+1)}\abs{g(\bar{k}^N)}^2.
	\end{aligned}
	\end{equation}
...
\begin{lemma}
	 Let $ N\geq2 $ and $ \xi\in H^{1/2}(\R^{3(N-1)}) $, then we have the following limits\begin{equation}
	 	\norm{\frac{1}{n}\int \diff k_N \mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)}^2_{L^2(\R^{3(N-1)})}\to 0,\text{ for }i\neq N\text{ and }n\to\infty,
	 \end{equation}
	 and \begin{equation}
		\norm{\xi(\bar{k}^i)-\frac{1}{4\pi n}\int \diff k_N \mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)}^2_{L^2(\R^{3(N-1)})}\to 0,\text{ for }i\neq N\text{ and }n\to\infty,
	 \end{equation}
\end{lemma}
\begin{proof}
	...
	\end{aligned}
	\end{equation}
	where $ A=1-\frac{N-2}{(m+1)(m+N-2)} $ and $ B=\frac{1}{m+1}-\frac{N-2}{(m+1)(m+N-2)} $, so $ 0<B<A<1 $ as above.\\
	For $ i=N $, we proceed by
	\begin{equation}
	\begin{aligned}
	\norm{\xi(\bar{k}^i)-\frac{1}{4\pi n}\int \diff k_N \mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)}^2_{L^2(\R^{3(N-1)})}\\
	=\int\diff \bar{k}^N\abs{\frac{1}{4\pi n}\left(4\pi n-\int_{B_n(0)}\diff k_N\hat{G}(k)\right)\xi(\bar{k}^N)}^2,\\
	=\int\diff \bar{k}^N\abs{\frac{1}{4\pi n}L^n(\bar{k}^N)\xi(\bar{k}^N)}^2.
	\end{aligned}
	\end{equation}
	Now remember that $ L^n(\bar{k}^N)<4\pi n $, but we also have from the definition of $ L $, \eqref{defL}, that \begin{equation}
	...
	\end{equation} 
	for $ \abs{\sum_{i=1}^{N-1}k_i}<R $. Thus we may estimate\begin{equation}
	\int\diff \bar{k}^N\abs{\frac{1}{4\pi n}L^n(\bar{k}^N)\xi(\bar{k}^N)}^2\leq C\int_{\abs{\bar{k}^N}<n}\diff\bar{k}^N L(\bar{k}^N)\abs{\xi(\bar{k}^N)}^2
	\end{equation}
\end{proof}
...
\begin{proof}
	...
	\begin{aligned}
	\norm{\xi^n(\bar{k}^N)}_{H^{1/2}(\R^{3(N-1)}) }\leq\frac{1}{4\pi n+\alpha}&\Bigg(\norm{\int_{B_n(0)}\diff k_N \hat{w}(k)}_{H^{1/2}(\R^{3(N-1)}) }\\&+\sum_{i=1}^{N}\norm{\int\diff k_N\left(\mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)\right)}_{H^{1/2}(\R^{3(N-1)}) }\Bigg).
	\end{aligned}
	\end{equation}
	By lemma \ref{lemmawH1-2} and \ref{lemmaGxi} we conclude that \begin{equation}
	\norm{\xi^n(\bar{k}^N)}_{H^{1/2}(\R^{3(N-1)}) }\leq C\norm{\xi(\bar{k}^N)}_{H^{1/2}(\R^{3(N-1)}) }
	\end{equation}
	for some $ C\geq0 $ depending only on $ N $, $ m $, and $ \mu $. Let now $ \xi^{n_j} $ be any subsequence of $ \xi^n $. Then we may conclude from the Banach-Alaoglu theorem that there exist a further subsequence $ \xi^{n_{j_k}} $ such that $ \xi^{n_{j_k}}\rightharpoonup \chi $ as $ k\to\infty $, for some $ \chi\in H^{1/2}(\R^{3(N-1)})  $. However then we necessarily have $ \xi^{n_{j_k}}\to \chi $ pointwise a.e. as $ k\to\infty $, but we know that $ \xi^{n_{j_k}}\to \xi $ pointwise a.e. as $ k\to\infty $, so we conclude that $ \chi=\xi $. Thus we infer that $ \xi^n\to\xi $ in $ H^{1/2}(\R^{3(N-1)})  $ norm as $ n\to\infty $.
	\end{proof}
	MISTAKE IN LAST PART (pointwise convergence)
\appendix
