...
We now give some generalization of results fromt the $ 1+1 $ case.
...
\end{lemma}
\begin{proof}
	The proof of the first two statements is a straightforward generalization of the proof of lemma \ref{H1 int} noticing that $ g=\norm{f(\bar{k}^N,\cdot)}_{L_2(\R^3)}\in L^2(\R^{3(N-1)}) $, for $ f\in L^2(\R^{3N}) $, by Tornelli's theorem. The last two statements simply follow by Cauchy-Schwartz and DCT. 
\end{proof}
A direct consequence is the following lemma \begin{lemma}
...
\begin{proof}
	...
	\begin{equation}
	\frac{1}{n}\left\lvert\int_{B_n(0)}\diff k_N\hat{w}(k)\right\rvert\leq\frac{1}{\sqrt{n}} \sqrt{1-\frac{\sqrt{\abs{\bar{k}^N}^2+1}}{n}\arctan\left(\frac{n}{\sqrt{\abs{\bar{k}^N}^2+1}}\right)}g(\bar{k}^N).
	\end{equation}
	Thus we may compute \begin{equation}
	\begin{aligned}
	&\norm{\frac{1}{n}\int_{B_n(0)}\diff k_N\hat{w}(k)}_{H^{1/2}(\R^{3(N-1)})}^2\\&=\frac{1}{n}\int\diff \bar{k}^N\sqrt{\abs{\bar{k}^N}^2+1}\left(1-\frac{\sqrt{\abs{\bar{k}^N}^2+1}}{n}\arctan\left(\frac{n}{\sqrt{\abs{\bar{k}^N}^2+1}}\right)\right)\abs{g(\bar{k}^N)}^2\\&
	\leq\frac{\sqrt{n^2+1}}{n}\norm{g}_{L^2(\R^{3(N-1)})}^2+\frac{1}{n}\int_{\abs{\bar{k}^N}>n}\diff k_N \sqrt{\abs{\bar{k}^N}^2+1}\frac{n^2}{(\abs{\bar{k}^N}^2+1)}\abs{g(\bar{k}^N)}^2\\&
	\leq\left(\frac{\sqrt{n^2+1}}{n}+1\right)\norm{g}_{L^2(\R^{3(N-1)})}^2.
	\end{aligned}
	\end{equation}
	Thus we now that $ h_n:=\frac{1}{n}\int_{B_n(0)}\diff k_N\hat{w}(k) $ is $ H^{1/2}(\R^{3(N-1)}) $ norm bounded. By the Banach-Alaoglu theorem, we may conclude that any subsequence $ h_{n_j} $ has a further subsequence $ h_{n_{j_k}} $ converging to $ h\in H^{1/2}(\R^{3(N-1)})  $. However, then $ h_n $ converges to $ h $ pointwise a.e., but by lemma \ref{lemmawH1} we know that $ h_n\to0 $ pointwise a.e., so we conclude that $ h=0 $. Thus we infer that $ h_n\to 0 $ in $ H^{1/2}(\R^{3(N-1)})  $ norm.
\end{proof}
...
\begin{proof}
	...
	\mathbbm{1}_{B_n(0)}(k_i)\frac{g(\bar{k}^i)}{g(\bar{k}^N)}=\mathbbm{1}_{B_n(0)}(k_i)\frac{\sqrt{k_i^2+K}}{\sqrt{k_N^2+K}}\leq\mathbbm{1}_{B_n(0)}(k_i)\frac{\sqrt{n^2+K}}{\sqrt{k_N^2+K}}\leq\mathbbm{1}_{B_n(0)}(k_i)\left(1+\frac{n}{\abs{k_N}}\right),
	\end{equation}
	and for $ \mathcal{H}=L^{2}(\R^{3(N-1)}) $ we have \begin{equation}
	\mathbbm{1}_{B_n(0)}(k_i)\frac{g(\bar{k}^i)}{g(\bar{k}^N)}=\mathbbm{1}_{B_n(0)}(k_i)\leq\mathbbm{1}_{B_n(0)}(k_i)\left(1+\frac{n}{\abs{k_N}}\right),
	...
	=C\int_{(0,n)}\diff r\ r^2 \int_{(0,\infty)}\diff q\ q^2\left(1+\frac{n}{\abs{k_N}}\right) \frac{1}{\left(A(q-\frac{B}{A}r)^2+Ar^2-\frac{B^2}{A}r^2+\mu\right)^{2}}\\&
	=C\int_{(0,n)}\diff r\ r^2 \int_{(-\frac{B}{A}r,\infty)}\diff \tilde{q}\ \left(\left(\tilde{q}+\frac{B}{A}r\right)^2+n\left(\tilde{q}+\frac{B}{A}r\right)\right) \frac{1}{\left(A\tilde{q}^2+Ar^2-\frac{B^2}{A}r^2+\mu\right)^{2}}\\&
	\lesssim C'\left(\int_{(0,n)}\diff r\ r^2\frac{1}{\sqrt{\left(A-\frac{B^2}{A}\right)r^2+\mu}}+\int_{(0,n)}\diff r\ r^2\frac{n}{\left(A-\frac{B^2}{A}\right)r^2+\mu}\right)\\&\leq\tilde{C}n^2
	\end{aligned}
	\end{equation}
...
\begin{proof}
	...
	\end{aligned}
	\end{equation}
	Actually this can be made more precise since we have by the triangle inequality \begin{equation}
	\begin{aligned}
	\norm{\xi^n(\bar{k}^N)}_{L^2(\R^{3(N-1)})}\leq\frac{1}{4\pi n+\alpha}&\Bigg(\norm{\int_{B_n(0)}\diff k_N \hat{w}(k)}_{L^2(\R^{3(N-1)})}\\&+\sum_{i=1}^{N}\norm{\int\diff k_N\left(\mathbbm{1}_{B_n(0)}(k_i)\hat{G}(k)\xi(\bar{k}^i)\right)}_{L^2(\R^{3(N-1)})}\Bigg).
	\end{aligned}
	\end{equation}
	By lemma \ref{lemmawH1} and \ref{lemmaGxi} we conclude that \begin{equation}
	\norm{\xi^n(\bar{k}^N)}_{L^2(\R^{3(N-1)})}\leq C\norm{\xi(\bar{k}^N)}_{L^2(\R^{3(N-1)})}
	\end{equation}
	for some $ C\geq0 $ depending only on $ N $, $ m $, and $ \mu $. Let now $ \xi^{n_j} $ be any subsequence of $ \xi^n $. Then we may conclude from the Banach-Alaoglu theorem that there exist a further subsequence $ \xi^{n_{j_k}} $ such that $ \xi^{n_{j_k}}\to \chi $ as $ k\to\infty $, for some $ \chi\in L^2(\R^{3(N-1)}) $. However then we necessarily have $ \xi^{n_{j_k}}\to \chi $ pointwise a.e. as $ k\to\infty $, but we know that $ \xi^{n_{j_k}}\to \xi $ pointwise a.e. as $ k\to\infty $, so we conclude that $ \chi=\xi $. Thus we infer that $ \xi^n\to\xi $ in norm as $ n\to\infty $.
