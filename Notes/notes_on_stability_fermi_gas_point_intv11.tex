...
%		
	...
	f_\mu^2=A_\mu\left(\mathcal{P}\frac{1}{k^2-\mu^2}\mathbbm{1}_{[0,n)}(k)+\frac{\left(\arctanh\left(\frac{\mu}{n}\right)+\frac{\alpha}{4\pi\mu}\right)}{\mu}\delta(k-\mu)\right),\quad \mu^2<n^2.
	\end{equation}
	The normalizationm $ A_\mu $, is left unspecified at this point, however the following theorem motivates a specific choice
	\begin{theorem}
		Let $ f_\mu^2 $ denote the functions in \eqref{EqGeneralizedEigenfunctions}. Let further $ A_\mu $ be defined by\begin{equation}
		\begin{aligned}
		A_\mu:=4 \pi \Bigg(\alpha ^2+8 i \pi ^3 \mu ^2 \log \left(\frac{4 n^2}{n^2-\mu
				...
				\end{aligned}
		\end{equation}
		we have the following inversion theorem\begin{equation}
		\int_{0}^{n}f_{\mu^2}(k)\int_{0}^{n}f_{\nu^2}(k) g(\nu) \nu^2\diff\nu k^2\diff k=g(\mu)
		\end{equation}
		for any function $ g\in L^1([0,n)) $. Furthermore, we have \begin{equation}
		\int_{0}^{n}f_{\nu^2}(k')\int_{0}^{n}f_{\nu^2}(k) h(k) k^2\diff k\nu^2\diff\nu=h(k')
		\end{equation}
	\end{theorem}
	\begin{proof}
		Proof needs. We have numerical evidence for the first inversion theorem, next step is to try second inversion numerically.
		If there is an $ A_\mu $ such that the theorem is true we must have 
		\begin{equation}
		\int_{0}^{n} \frac{1}{A_\nu}f_{\nu^2}(k) \left(\int_{0}^{n}\frac{1}{A_\mu}f_{\mu^2}(k)\mu^2\diff\mu\right)k^2\diff k=\frac{1}{A_\nu^2},
		\end{equation}
		however, this is an equation for $ A_\nu $ since $ \frac{1}{A_\mu}f_{\mu^2}(k) $ is known and independent of $ A_\mu $.
	\end{proof}
	\appendix
