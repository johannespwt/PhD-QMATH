...
\end{equation}
We know from section \ref{SectionRankOnePerturbation} that $ H_1:=\frac{m}{m+1}\sum_{i=1}^{N}k_i^2-\gamma_R\sum_{i=1}^{N}\braket{\hat{\phi}_i\vert \cdot}\hat{\phi}_i $ have a spectrum with (at most) one negative eigenvalue, and continuous spectrum consisting of $ (0,\infty) $. Thus we may write $ H $ as \begin{equation}
...
\end{equation}
where $ P_0=\ket{0}\bra{0} $ is the projector onto the ground state. Thus we see that $ \sum_{m=1}^{3}\sum_{j,k}\left(a^m_{0j}a^m_{k0}c^\dagger_kc_j\right) $ is the second quantization of the operator $ \sum_{m=1}^{3}k^mP_0k^m $. Thus we clearly have \begin{equation}
(1-2N_0)\sum_{m=1}^{3}\sum_{j,k}\left(a^m_{0j}a^m_{k0}c^\dagger_kc_j\right)\geq -\sum_{m=1}^{3}k^mP_0k^m\geq -P_0^\perp k^2P_0^\perp.
\end{equation}
