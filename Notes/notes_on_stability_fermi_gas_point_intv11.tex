...
Collecting all terms and using estimate \eqref{Eqone0estimate} with $ \epsilon=1/2 $, we see that \begin{equation}
H\geq \sum_{i}\tilde{\varepsilon}_i N_i-\frac{3}{m+1}\left(1-\frac23 N_0\right)\sum_{m=1}^{3}\sum_{i,j}a_{i0}^ma_{0j}^m c_i^\dagger c_j+\frac{1}{m+1}\sum_{m=1}^{3}\sum_{i,j}\left(a_{0j}^ma_{ji}^mc_0^\dagger c_i+a_{ij}^ma_{j0}^mc_i^\dagger c_0\right)
\end{equation}
	\appendix
