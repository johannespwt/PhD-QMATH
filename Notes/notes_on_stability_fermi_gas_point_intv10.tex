...
%		
	...
	\end{equation}
	where the $ 4\pi $ comes from the normalization of $ \mathbbm{1}_{B_n}=\sqrt{4\pi}\mathbbm{1}_{[0,n)}(k)\otimes Y_0^0 $. In order to solve the generalized eigenvalue equation, we consider first the case $ k=0 $, where the equation reads\begin{equation}
	...
	\end{equation}
	where $ \mathcal{P} $ denotes the Cauchy principal value. Notice that \eqref{EqParticularSol} is self-consistent for any value of $ f(0) $.
\appendix
