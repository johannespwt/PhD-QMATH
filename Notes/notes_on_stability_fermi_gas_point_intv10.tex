...
%		
	...
	\end{equation}
	where the $ 4\pi $ comes from the normalization of $ \mathbbm{1}_{B_n}=\sqrt{4\pi}\mathbbm{1}_{[0,n)}(k)\otimes Y_0^0 $. In order to solve the generalized eigenvalue equation, we consider first the case $ k=0 $, where the equation reads\begin{equation} \label{EqConsistency}
	...
	\hat{f}(k)=\mathcal{P}\frac{-\lambda f(0)}{k^2-\lambda},
	\end{equation}
	where $ \mathcal{P} $ denotes the Cauchy principal value. Notice that \eqref{EqParticularSol} is self-consistent for any value of $ f(0) $. Thus in order to have consistency we need only that the solution satisfies \eqref{EqConsistency}. Notice that \begin{equation}
	\hat{f}(k)=\mathcal{P}\frac{-\lambda f(0)}{k^2-\lambda}+B\delta(k-\sqrt{\lambda}),
	\end{equation}
	satisfies is consistent in the way $ f(0)=f(0) $ and by adjusting $ B $ appropriately is also satisfies \eqref{EqConsistency}. 
\appendix
