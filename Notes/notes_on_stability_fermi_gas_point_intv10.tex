...
%		
	...
	\hat{f}(k)=\mathcal{P}\frac{-\lambda \hat{f}(0)\mathbbm{1}_{[0,n)}(k)}{k^2-\lambda},
	\end{equation}
	where $ \mathcal{P} $ denotes the Cauchy principal value. Notice that \eqref{EqParticularSol} is self-consistent for any value of $ \hat{f}(0) $. Thus in order to have consistency we need only that the solution satisfies \eqref{EqConsistency}. Notice that \begin{equation}
	\hat{f}(k)=\mathcal{P}\frac{-\lambda \hat{f}(0)}{k^2-\lambda}\mathbbm{1}_{[0,n)}(k)+\frac{B}{\sqrt{\lambda}}\delta(k-\sqrt{\lambda}),
	\end{equation}
	is consistent in the way $ \hat{f}(0)=\hat{f}(0) $ and by adjusting $ B $ appropriately is also satisfies \eqref{EqConsistency}. Thus by fixing $ B $ such that \eqref{EqConsistency} is satisfied, $ (\hat{f},\lambda) $ is the solution to the generalized eigenvalue problem for this Hamiltonian. Thus we calculate \begin{equation}
	\begin{aligned}
	\lambda \hat{f}(0)=-4\pi\gamma_\alpha&\int_{0}^{n}\diff r r^2 \left(\mathcal{P}\frac{-\lambda \hat{f}(0)}{r^2-\lambda}+\frac{B}{\sqrt{\lambda}}\delta(r-\sqrt{\lambda})\right)\\&=-\frac{1}{n+\frac{\alpha}{4\pi}}\left(-\lambda \hat{f}(0)\left[n-\sqrt{\lambda}\arctanh\left(\frac{\sqrt{\lambda}}{n}\right)\right]+\sqrt{\lambda} B\right)
	\end{aligned}
	\end{equation}
	Thus setting $ B=-\lambda \hat{f}(0)\left(\arctanh\left(\frac{\sqrt{\lambda}}{n}\right)+\frac{\alpha}{4\pi\sqrt{\lambda}}\right) $, we have consistency. Thereby we conclude that \begin{equation}
	\hat{f}_\lambda(k)=A\left(\mathcal{P}\frac{1}{k^2-\lambda}\mathbbm{1}_{[0,n)}(k)+\frac{\left(\arctanh\left(\frac{\sqrt{\lambda}}{n}\right)+\frac{\alpha}{4\pi\sqrt{\lambda}}\right)}{\sqrt{\lambda}}\delta(k-\sqrt{\lambda})\right),
	\end{equation}
