...
\end{aligned}
\end{equation}
Assuming that $ \dom{H_\alpha} $ is dense in $ \dom{F_\alpha} $ we then have $ \dom{H_\alpha^*}=\dom{H_\alpha} $ and the operator, $ H_\alpha $, is self-adjoint. It is a general fact that $ \dom{H_\alpha} $ is dense whenever $ F_\alpha $ is closed. Thus we are now ready to calculate the Hamiltonian of the quadratic form $ F_\alpha $. Notice that by the definition of $ \dom{H_\alpha} $ we must have that for $ v\in\dom{H_\alpha} $ and $  (u_n)_{n\geq1}\subset\dom{F_\alpha} $  such that $ u_n\to0 $ in $ L_2(\R^3) $ it holds that  $ F_\alpha(v,u_n)\to 0 $. Thus by writing $ v=h+\chi G $ with $ h\in H_1(\R^3) $ and $ \chi\in\C $ and $ u_n=w_n+\xi G\in\dom{F_\alpha} $, with $ w_n\in H_1(\R^3) $ such that $ w_n\to-\xi G $ in $ L_2(\R^3) $ we have \begin{equation}
F(v,u)=\int_{\R^3} d^3k\ \left(k^2+\mu\right)\overline{\hat{h}(k)}\hat{w}_n(k)-\mu\braket{v,u_n}_{L_2(\R^3)}+\bar{\chi}\xi\left(\alpha+2\pi^2\sqrt{\mu}\right).
\end{equation}
We immediately see that for the first term to be $ L_2(\R^3) $ bounded in $ w $ we must have that $ h\in H_2(\R^3) $. Secondly since the first term is $ L_2(\R^3) $ bounded we must have \begin{equation}
\int_{\R^3} d^3k\ \left(k^2+\mu\right)\overline{\hat{h}(k)}\hat{w}_n(k)\to-\int_{\R^3} d^3k\ \left(k^2+\mu\right)\overline{\hat{h}(k)}\xi \hat{G}(k)=-\xi\int_{\R^3} d^3k\ \overline{\hat{h}(k)}\quad \text{as }n\to\infty.
\end{equation} 
...
\begin{lemma}
	Let $ h\in H_2(\R^3) $ then $ h $ is continuous and $ \frac{1}{(2\pi)^{3/2}}\int_{\R^3}d^3k\ \hat{h}(k)e^{ik\cdot x}=h(x) $.
\end{lemma}
\begin{proof}
	First that $ h $ is continuous follows from Sobolev's embedding theorem. Next notice that since $ h\in H_2(\R^3) $ we must have that \begin{equation}
	\hat{h}(k)=\frac{\hat{f}(k)}{|k|^2+1},
	\end{equation}
	for some $ \hat{f}\in L_2(\R^3) $. Thus we also see that $ \hat{h} $ is clearly in $ L_1(\R^3) $ by H\"older's inequality and the fact that $ \frac{1}{|k|^2+1}\in L_2(\R^3) $ and therefore $ \check{\hat{h}} $ is bounded and continuous. By Fourier's inversion theorem we have that $ \check{\hat{h}}=h $ a.e, and by continuity of $ h $, we conclude that $ \check{\hat{h}}=h $. Since $ \hat{h}\in L_1(\R^3) $ this amounts to \begin{equation}
	\frac{1}{(2\pi)^{3/2}}\int_{\R^3}d^3k\ \hat{h}(k)e^{ik\cdot x}=h(x) 
	\end{equation}
\end{proof}
Now given the above lemma we clearly see that \begin{equation}
\xi\int_{\R^3} d^3k\ \overline{\hat{h}(k)}=(2\pi)^{3/2}\xi\overline{h(0)}.
\end{equation}
Thereby we find the condition on the domain\begin{equation}
-(2\pi)^{3/2}\xi\overline{h(0)}+\bar{\chi}\xi(\alpha+2\pi^2\sqrt{\mu})=0
\end{equation}
correpsponding to the boundary condition $ h(0)=(2\pi)^{-3/2}\xi(\alpha+2\pi^2\sqrt{\mu}) $. Now turning to the action of the operator $ H_\alpha $ it is easier to consider $ H_\alpha+\mu $ since we then have \begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
notice that $ u $ and $ v $ switched roles at some point, which does not change anything.
