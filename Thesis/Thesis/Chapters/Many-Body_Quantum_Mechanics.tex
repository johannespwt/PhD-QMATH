...
\begin{lemma}
	Let $ (k_i)_{i=1}^{N} $ be the with $ k_1<k_2<\ldots<k_N $ satisfy \eqref{EqPseudoMomentaDifferenceExpansion}. Then we have $$k_{i+1}-k_i\geq \frac{2\pi}{L}\left(1+2\frac{\rho}{c}\right)^{-1}+\rho \mathcal{O}\left(1/(cL)^2\right). $$
\end{lemma}
\begin{proof}
	This follows directly by discarding the term $ (k_s-k_i)^2 $ in the denominator inside the sum in \eqref{EqPseudoMomentaDifferenceExpansion}.
\end{proof}
It follows straightaway for the ground state where $k_1=-k_N$ that \begin{equation}
	\abs{k_n}\geq \frac{2\pi}{L}\left(1+2\frac{\rho}{c}\right)^{-1}n+\rho \mathcal{O}\left(1/(cL)^2\right).
\end{equation} 
Therefore we find the lower bound on the energy \begin{equation}
	\sum_{n=1}^{N}k_n^2\geq N\left(\rho+\frac{1}{L}\right)\left(\rho+\frac{2}{L}\right)\frac{\pi^2}{3}\left(1+2\frac{\rho}{c}\right)^{-2}+\mathcal{O}\left(\frac{\rho}{c}\frac{\rho^2}{cL} \right).
\end{equation}
