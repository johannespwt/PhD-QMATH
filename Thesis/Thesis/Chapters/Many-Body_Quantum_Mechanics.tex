...
\end{definition}
Of course $ \mathcal{E}_{(v,\sigma)}\geq 0 $ for any $ \sigma\in[0,\infty] $ and $ v\geq 0 $. However, the closability of $ \mathcal{E}_{(v,\sigma)} $ is not evident. In fact we will have to...
\begin{remark}
	For example, any radial $ v\geq 0 $ (possibly a measure) with compact support is  $\mathcal{E}_{(v,\sigma)} $ is the quadratic form associated to a self-adjoint operator on some Hilbert space $ \mathcal{H}_{(v,\sigma)}\subset L^2_{s/a}(\Omega^N) $. To see this notice that $ \mathcal{H}_{(v,\sigma)}:=\overline{\dom{\mathcal{E}_{(v,\sigma)}}}^{\norm{\cdot}_2}\in L^2_{s/a}(\Omega^N)$ is a closed subspace, and thus itself a Hilbert space. Furthermore, it is well known that $\mathcal{E}_{(0,\sigma)}$ is closable on $  \mathcal{H}_{(v,\sigma)} $. Thus closability of $ \mathcal{E}_{(v,\sigma)} $ amount to showing that $ \psi_n \xrightarrow{\norm{\cdot}_2}0 $ as $ n\to\infty $ and $ ((\sum_{i<j}v(x_i-x_j))^{1/2}\psi_n)_{n\in\mathbb{N}}\subset L^2(\Omega^N) $ Cauchy, implies $ (\sum_{i<j}v(x_i-x_j))^{1/2}\psi_n\xrightarrow{\norm{\cdot}_2}0  $. This is evident from the fact $ (\sum_{i<j}v(x_i-x_j))^{1/2}\psi_n $ that $\psi_n\xrightarrow{\norm{\cdot}_2}f$ for some $ f\in L^2(\Omega^N) $ by completeness. Now $ \psi_n $ has a subsequence that converges almost everywhere to $ 0 $, and this subsequence further has a subsequence that converges almost everywhere to $ f $. Hence $ f=0 $ almost everywhere. \\
	Thus there is a corresponding self-adjoint operator $ H_{(v,\sigma)} $ to $ \mathcal{E}_{(v,\sigma)} $ on $ \mathcal{H}_{(v,\sigma)} $, which we shall refere to as the Hamiltonian.
\end{remark}
