...
2\pi f(k)-1=2c\int_{-K}^{K}\frac{f(p)}{c^2+(p-k)^2}\diff p+\mathcal{O}(1/(cL).
\end{equation}
...
The model of interest describes $ N $ spin--$ 1/2 $ fermions and is given using the same formal Hamiltonian as for the Lieb-Liniger model\begin{equation}
H_{YG}=-\sum_{i=1}^{N}\partial_i^2+2c\sum_{1\leq i<j\leq N}\delta(x_i-x_j),
\end{equation}
however the domain is taken to some spin sector. To analyze the problem, Yang considers the possible spacial symmetries that may appear problem. Having combined spin-space anti-symmetry, requires that any irreducible representation of $ S_N$ labeling the spacial symmetry, must have a corresponding conjugate irreducible representation labeling the spin symmetry. As an example consider the two particle case where the wave function is either symmetric and the spin state is the singlet, \emph{or} the wave function is anti-symmetric and the spin state is in the triplet. If you have more particles, the picture is a bit more complicated. Notice that one can not have $ 3 $ spin--$ 1/2 $ particles that are mutually in the singlet state with each other. It turns out, that the smart way to label the symmetry of a spin state is by Young tableaux. A tableau label first which particles are in a symmetric state by having their particle labels in the same row and then which particles are in the in singlet state with each other by having the two particles labels in the same column. Thus \begin{equation}
	\begin{ytableau}
	1 & 2 \\
	 3 &  \none 
	\end{ytableau}=\operatorname{P}_{\ket{\uparrow \uparrow \downarrow }-\ket{\downarrow \downarrow \uparrow}}
\end{equation}
...
1 & 3 \\
2 &  \none 
\end{ytableau}=\operatorname{P}_{\ket{\uparrow \downarrow \uparrow }-\ket{\downarrow \uparrow \downarrow}}
\end{equation}
...
\begin{ytableau}
1 & 2& 3 
\end{ytableau}=\operatorname{P}_{\ket{\uparrow \uparrow \uparrow },\ket{\downarrow\downarrow\downarrow},\ket{\uparrow\downarrow\uparrow}+\ket{\downarrow\uparrow\uparrow}+\ket{\uparrow\uparrow\downarrow},\ket{\uparrow\downarrow\downarrow}+\ket{\downarrow\downarrow\uparrow}+\ket{\downarrow\uparrow\downarrow}}
\end{equation}
