...
\end{equation}
Taking the logarithm of \eqref{EqYGSpinRapidityRelation} and \eqref{EqYGPeriodicSpectrumCondition} adding certain intergers to get a well defined $ c\to\infty $ limit as we did in Section \ref{SectionLiebLinigerModel}, one finds \begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Going to the thermodynamic limit, \ie $ N,M,L\to\infty $ proportionally, on then find the equations for the energy \begin{equation}
\begin{aligned}
2\pi\sigma(\Lambda)&=-\int_{-B}^{B}\frac{2c\sigma(\Lambda')\diff \Lambda'}{c^2+(\Lambda-\Lambda')^2}+\int_{-Q}^{Q}\frac{4c\rho\diff \rho}{c^2+4(k-\Lambda)^2}\\
2\pi\rho(k) &= 1+\int_{-B}^{B}\frac{4c\sigma(\Lambda')\diff \Lambda'}{c^2+4(k-\Lambda')^2}\\
N/L=\int_{-Q}^{Q}\rho(k) \diff k,\quad&\quad M/L=\int_{-B}^{B}\sigma(\Lambda)\diff\Lambda,\\
E/L&=\int_{-Q}^{Q}k^2\rho(k)\diff k.
\end{aligned}
\end{equation}
