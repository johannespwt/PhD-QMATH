...
\begin{theorem}[Lieb and Mattis, \cite{lieb1962theory}, for periodic boundary conditions, $ d=1 $]
...
\end{theorem}\label{TheoremLiebMattis}
\begin{proof}
...
\end{proof}} tells us that the eigenvalue $ \mu_j $ yielding the smallest eigenvalue of \eqref{EqYGHamiltonian} must come from the total spin sector $ N-2M $, \ie minimal total spin in the case when $ N/2 $ is an odd integer.\\
...
In order to lower bound the ground state energy of Yang-Gaudin model, we do need to justify that the ground state is reached by the Yang-Bethe ansatz states. This is justified the following result 
\begin{proposition}
	Let $ N=4n+2 $ for some integer $ n $ and let $ \Psi_V(c) $ denote the variational ground state among the Yang-Bethe ansatz states then $ \Psi_V(c) $ is the true ground state of the Yang-Gaudin model \ref{EqYGHamiltonian}.
\end{proposition}
\begin{proof}
	By the theorem of Lieb and Mattis \cite{lieb1962theory} (see Theorem \ref{TheoremLiebMattis}), we have that the ground state at any finite $ c\geq 0 $ is unique. 
%	On the other hand, letting $ \Psi_V^M(c) $ denote the Bethe-Yang ansatz ground state with total $ S_{\text{tot.}}^z=\frac{1}{2}(N-2M) $ we may verify that $ \lim\limits_{c\to0}\Psi^M_V(c)=\Psi_F $ for any $ M $. For each of these, we may find a total $ S^z_{\text{tot.}}=0 $ state, with the same energy, by applying $ S_{\text{tot.}}^- $ $ N-2M $ times, \ie $(S_{\text{tot.}}^-)^{N-2M}\Psi_V^{M}  $.
	On the other hand, one may verified that $ \lim\limits_{c\to0}\Psi_V(c)=\Psi^{1/2}_F $ in $ L^2 $, with $ \Psi^{1/2}_F $ denoting the free spin--$ 1/2 $ Fermi ground state, \ie a spin-space Slater determinant (for this, one needs an approach by Gaudin \cite{gaudin1967systeme,gaudin2014bethe} to study the $ c\to 0 $ limit of Bethe-Yang ansatz solution). However, then $ \Psi_V(c) $ must be the true ground state, since having an orthogonal true ground state, would imply a degenerate ground state at $ c=0 $, which is a contradiction. 
\end{proof}
